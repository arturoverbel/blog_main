...
class WpdiscuzOptions implements WpDiscuzConstants {
	...
	public function saveOptions() {
		if (isset($_POST["wc_submit_options"]) && !empty($_POST["wpd_tab"])) {
			...
			if (self::TAB_FORM === sanitize_text_field($_POST["wpd_tab"])) {
				...
				$this->moderation["commentEditableTime"]          = isset($_POST[self::TAB_MODERATION]["commentEditableTime"]) ? absint($_POST[self::TAB_MODERATION]["commentEditableTime"]) : 900;
	...
	private function addonNote() {
		if ((!empty($_GET["page"]) && in_array($_GET["page"], [
			...
			$currentHash   = "BuddyPress Integration, Tenor GIFs Integration, Voice Commenting, GIPHY Integration";
...
<!--                            </div>-->
<!--                        --><?php //endif; ?>
	                    <?php if(!in_array("Voice Commenting", $lastHashArray)): ?>
                            <div style="display:inline-block; min-width:27%; padding-right:10px; margin-bottom:10px;">
                                <img src="<?php echo plugins_url(WPDISCUZ_DIR_NAME . "/assets/addons/voice/header.png"); ?>" style="height:50px; width:auto; vertical-align:middle; margin:0px 10px; text-decoration:none; float: left;"/>
                                <a href="https://gvectors.com/product/wpdiscuz-voice-commenting/" target="_blank" style="color:#444; text-decoration:none;" title="<?php esc_attr_e("Go to the addon page", "wpdiscuz"); ?>">wpDiscuz - Voice Commenting <br><span style="margin: 0; font-size: 12px; line-height: 15px; display: block; padding-top: 5px;">Allows to discuss with your voice in the comment section. Adds a microphone button to the comment form.</span></a>
                            </div>
	                    <?php endif; ?>
...
<!--				        --><?php //endif; ?>
	...
	public function addonHash() {
		$viewed = "BuddyPress Integration, Tenor GIFs Integration, Voice Commenting, GIPHY Integration";
